...
window.FITTRACK.screens.renderHistory = async function(container) {
  container.innerHTML = `
    ...
    <div id="history-pagination-container" class="mt-4 flex-row justify-center d-none">
      <button id="btn-load-more-history" class="btn btn-secondary py-3 px-6 rounded-2xl text-sm font-bold" style="width:100%; max-width:320px; border:1px solid var(--color-border); background:var(--color-surface-2);">
  ...
  try {
    const allHistory = await window.FITTRACK.getWorkoutHistory(100);
          ...
          if (ok) {
            try {
              ...
              if (cardEl) {
                ...
                setTimeout(() => {
                  cardEl.remove();
                  const remainingCards = document.querySelectorAll('#history-list .card');
    ...
    function renderList() {
      const slice = allHistory.slice(0, visibleCount);
      listEl.innerHTML = slice.map(renderWorkoutCard).join('');
      bindDeleteButtons();
      if (window.lucide) lucide.createIcons();

      if (visibleCount < allHistory.length) {
